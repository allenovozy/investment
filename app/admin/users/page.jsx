...
const Users = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch("https://72.60.93.14/server/Api/Admin/users/list.php", {
        credentials: "include",
      })
  ...
  const handleAdjustBalance = async (e) => {
    e.preventDefault()
    try {
      const res = await fetch("https://72.60.93.14/server/Api/Admin/users/update_balance.php", {
        method: "POST",
        credentials: "include",
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      const res = await fetch("https://72.60.93.14/server/Api/Admin/users/delete.php", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  const handleViewUser = async (user) => {
    try {
      const res = await fetch(`https://72.60.93.14/server/Api/Admin/users/activity.php?id=${user.id}`, {
        credentials: "include",
      })
      const data = await res.json()
      if (data.success) {
...
const handleEditSubmit = async (e) => {
  e.preventDefault()
  try {
    const res = await fetch("https://72.60.93.14/server/Api/Admin/users/edit.php", {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
