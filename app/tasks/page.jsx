...
export default function TasksPage() {
  ...
  const fetchUser = async () => {
    const res = await fetch("https://185.27.134.59/server/Api/dashboard.php", {
      credentials: "include",
    })
  ...
  const fetchTasks = async () => {
    const res = await fetch("https://185.27.134.59/server/Api/tasks.php", {
      credentials: "include",
    })
    const data = await res.json()
    if (data.success) {
          ...
          <>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-surface rounded-xl p-5">
                <p className="text-muted text-sm mb-1">Active Tasks</p>
                ...
                {tasks.length === 0 ? (
                  ...
                  tasks.map((task) => {
                    ...
                    return (
                      <div
                        key={task.id}
                        className="flex items-center gap-4 p-4 rounded-xl border border-border bg-surface-light"
                      >
